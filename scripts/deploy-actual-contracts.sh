...
setup_keys() {
    ...
    local mnemonic
    if mnemonic=$(kurtosis service exec "$ENCLAVE_NAME" thorchain-node "cat /tmp/mnemonic" 2>/dev/null); then
        echo "✓ Retrieved prefunded mnemonic"
        
        thornode keys delete demo-key --keyring-backend test --yes 2>/dev/null || true
        echo "$mnemonic" | thornode keys add demo-key --recover --keyring-backend test
        
        local address
        address=$(thornode keys show demo-key --keyring-backend test -a)
...
deploy_counter_contract() {
    ...
    local upload_result
    upload_result=$(thornode tx wasm store "$PROJECT_ROOT/build/counter.wasm" \
        --from demo-key \
    ...
    local code_id
    code_id=$(thornode query wasm list-code --node "$RPC_ENDPOINT" --output json | jq -r '.code_infos[-1].code_id')
    echo "✓ Counter contract code ID: $code_id"
    
    echo "Instantiating counter contract..."
    local instantiate_result
    instantiate_result=$(thornode tx wasm instantiate "$code_id" '{"count": 42}' \
        --from demo-key \
        --keyring-backend test \
        --chain-id thorchain \
        --node "$RPC_ENDPOINT" \
    ...
    local contract_addr
    contract_addr=$(thornode query wasm list-contract-by-code "$code_id" --node "$RPC_ENDPOINT" --output json | jq -r '.contracts[0]')
    echo "✓ Counter contract address: $contract_addr"
    
    echo "Testing counter contract..."
    local count_result
    count_result=$(thornode query wasm contract-state smart "$contract_addr" '{"get_count":{}}' --node "$RPC_ENDPOINT" --output json)
    local count_value
...
deploy_cw20_contract() {
    ...
    local upload_result
    upload_result=$(thornode tx wasm store "$PROJECT_ROOT/build/cw20-token.wasm" \
        --from demo-key \
        --keyring-backend test \
        --chain-id thorchain \
        --node "$RPC_ENDPOINT" \
...
EOF
    ...
    local instantiate_result
    instantiate_result=$(thornode tx wasm instantiate "$code_id" "$instantiate_msg" \
        --from demo-key \
        --keyring-backend test \
        --chain-id thorchain \
        --node "$RPC_ENDPOINT" \
    ...
    balance_result=$(thornode query wasm contract-state smart "$contract_addr" "{\"balance\":{\"address\":\"$demo_address\"}}" --node "$RPC_ENDPOINT" --output json)
    local balance_value
...
test_contract_interactions() {
    ...
    if [ -f "$PROJECT_ROOT/counter-contract-address.txt" ]; then
        ...
        local new_count
        new_count=$(thornode query wasm contract-state smart "$counter_addr" '{"get_count":{}}' --node "$RPC_ENDPOINT" --output json | jq -r '.data.count')
    ...
    if [ -f "$PROJECT_ROOT/cw20-contract-address.txt" ]; then
        ...
        local token_info
        token_info=$(thornode query wasm contract-state smart "$cw20_addr" '{"token_info":{}}' --node "$RPC_ENDPOINT" --output json)
        local token_name
